                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  for(int f_y = 0; f_y < filterWidth; f_y++) {
    for(int f_x = 0; f_x < filterWidth; f_x++) {
        int n_x = thread_2D_pos.x +f_x - filterWidth/2;
        int n_y = thread_2D_pos.y +f_y - filterWidth/2;
        n_x = min(max(n_x,0),numCols-1);
        n_y = min(max(n_y,0),numRows-1);
        float weight = filter[f_y*filterWidth + f_x];
        res += weight*static_cast<float>(inputChannel[n_y*numCols + n_x]);
    }
  }
